    ...
    <>
      <li>
        <NavLink to="/dashboard">
          {({ isActive }) => (
            <button className="relative flex items-center justify-center w-full gap-2 px-2 font-semibold text-white rounded-t-md cursor-pointer md:justify-start md:bg-background/50 md:px-3 h-9">
              <Layers className="md:size-5" />{' '}
              <span className="hidden md:block">Overview</span>
              {/* underline */}
              {isActive && (
                <span className="absolute w-1/3 h-1.4 rounded-full md:hidden -bottom-2 bg-zinc-100"></span>
              )}
            </button>
          )}
        </NavLink>
